...
const AssignMentor = () => {
  ...
  return (
    <div className="mx-auto md:w-11/12">
        ...
        <p className="px-3 py-1 mb-1 font-medium text-center rounded-md bg-slate-300">Assign Mentor for Student</p>
        <div className="flex flex-col flex-wrap items-center justify-center gap-8 px-5 py-8 bg-amber-700">
            {
                <>
                    <MentorList 
...
function StudentList({checkedItems, setCheckedItems}){
    ...
    useEffect(() => {
        const URL = noMent ? `${API}/${AllStuEndPoints}` : `${API}/${NoMentStuEndPoints}`
        fetch(URL, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            ...
            <div>
                {
                    ...
                    (
                        <div>
                            ...
                            <p className="mb-3 font-semibold">All Students List can change mentor</p>
                        </div>
                    ) :
                    (
                        <div>
                            ...
                            <p className="mb-3 font-semibold">Assign new Mentor</p>
                        </div>
                    )
                }
            ...
            <div className="flex flex-row flex-wrap gap-5 px-3 py-2 text-lg max-w-screen justify-normal">
                {
                    stuData?.map((val) => 
                    <label key={val._id}
                    className="px-3 py-1 bg-slate-500"
                    > 
                        <input 
                        type="checkbox" 
                        value={val._id} 
...
function MentorList({selectedMentor, setSelectedMentor}){
    ...
    return(
        <div className="flex flex-col">
            <label htmlFor="mentor" className="font-semibold">Select Mentor</label>
            <select name="mentor" id="mentor" value={selectedMentor} onChange={handleMentorChange}>
                <option value={''} >Select Mentor</option>
                {
                    mentData?.map((val) => <option key={val._id} value={val._id}>{val.mentor_name}</option> )
                }
            </select>
        </div>
    )
